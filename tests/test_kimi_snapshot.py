...
BUILTIN_TOOL = Tool(name="$web_search", description="Search the web", parameters={"type": "object", "properties": {}})
    ...
    **COMMON_CASES,
    ...
    "assistant_with_reasoning": {
        "history": [
            ...
            Message(role="assistant", content=[ThinkPart(think="Let me think..."), TextPart(text="The answer is 4.")]),
        ...
        assert results == snapshot(
            {
                ...
                "assistant_with_tool_call": {
                    "messages": [
                        ...
                        {"role": "assistant", "content": "I'll add those numbers for you.", "tool_calls": [{"type": "function", "id": "call_abc123", "function": {"name": "add", "arguments": '{"a": 2, "b": 3}'}}]},
                    ],
                    "tools": [],
                },
                "tool_result": {
                    "messages": [
                        {"role": "user", "content": "Add 2 and 3"},
                        {"role": "assistant", "content": "", "tool_calls": [{"type": "function", "id": "call_abc123", "function": {"name": "add", "arguments": '{"a": 2, "b": 3}'}}]},
                ...
                "assistant_with_reasoning": {
                    "messages": [
                        {"role": "user", "content": "What is 2+2?"},
                        {"role": "assistant", "content": "The answer is 4.", "reasoning_content": "Let me think..."},
...
async def test_kimi_generation_kwargs():
    with respx.mock(base_url="https://api.moonshot.ai") as mock:
        mock.post("/v1/chat/completions").mock(return_value=Response(200, json=make_response()))
        provider = Kimi(model="kimi-k2-turbo-preview", api_key="test-key", stream=False).with_generation_kwargs(temperature=0.7, max_tokens=2048)
...
async def test_kimi_with_thinking():
    with respx.mock(base_url="https://api.moonshot.ai") as mock:
        mock.post("/v1/chat/completions").mock(return_value=Response(200, json=make_response()))
        provider = Kimi(model="kimi-k2-turbo-preview", api_key="test-key", stream=False).with_thinking("high")
